...
`forge/workbench/plan.yaml` is an internal framework artifact in V2. This skill owns it; the public CLI does not expose a separate planning command.
...
- If bootstrap is still too vague to order real work, route back to `forge-discover` or `forge-spec`.
- If the plan depends on unnamed operations, missing verification items, or unclear runtime boundaries, stop and send those gaps back to `forge-spec`.
- If repo-layout uncertainty changes execution order materially, resolve that before finalizing the plan; do not bury it in notes.
- If the human asks to start coding while the first slice is still ambiguous, stop and finalize the plan first.
