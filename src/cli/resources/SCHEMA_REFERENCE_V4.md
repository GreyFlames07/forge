...
Forge has three central schema files, one crawlable decision log, and code-owned
C3 annotations.
...
- There is no `relationships` section; relationships are inferred from adjacent
  runtime steps whose `container` values differ.
- Flow diagrams render runtime containers as nodes and label edges with the step
  ids that move control between containers.
- A container-flow step belongs to exactly one runtime container.
- Multiple steps may share the same `id`; same-id steps are concurrent runtime
  component work in that control-flow phase.
- Adjacent steps with different `id` values should not repeat the same
  `container`; merge them into one runtime step and describe the internal work
  with `logic`, `branches`, and local/component operations.
- A step has `next`, `branches`, or neither. It must not have both `next` and
  `branches`.
- `branches` describes alternative control-flow targets from the current runtime
  step; it does not describe local operation ordering or concurrency.
...
decisions:
  - id: string
    date: YYYY-MM-DD
    owner_skill: forge-schema | forge-review | forge-security | forge-build
    scope: system | container | flow | entity | component | operation | build | security | testing
    refs:
      - string
    decision: string
    rationale: string
...
- Record non-trivial decisions that affect architecture, security, contracts,
  source-root layout, implementation direction, test strategy, or accepted risk.
- Keep each entry small and factual.
- Use stable `snake_case` ids.
- `refs` should point to Forge ids where possible, such as a business action,
  container, container flow, entity, component, operation, or source root.
- Do not use the decision log as a scratchpad or meeting transcript.
- When a decision changes, add a new entry with `supersedes`; do not rewrite
  history unless correcting a factual typo.
...
logic:
  - string
participates_in:
  - container_flow: flow_id:step_number
    local_flow: local_flow_id:step_number
    passes?: string
    flow_logic?:
      - string
    next?: number
...
participates_in:
  - container_flow: update_account:1
    local_flow: update_account_backend:1
...
- `participates_in` describes where the operation appears in one or more
  container flows and component operation sequences.
- `container_flow` in `participates_in` uses `flow_id:step_number`, where the
  suffix is the runtime container-flow step the operation participates in.
- `local_flow` in `participates_in` uses `local_flow_id:step_number`, where the
  suffix is the operation's step inside that component operation sequence.
- The container flow contains the component operation slices. Each component
  operation sequence is rendered inside the runtime step referenced by
  `container_flow` and ordered by `local_flow` step.
- Multiple operations may share the same `local_flow` step suffix; same-step
  operations are rendered as concurrent component operation work.
- `passes` is the conceptual payload handed to the next component operation step.
- `flow_logic` is optional flow-specific context; it must not contradict the
  operation's core `logic`.
- `next` and `branches` inside `participates_in` entries describe component
  operation control flow within the same component operation sequence.
